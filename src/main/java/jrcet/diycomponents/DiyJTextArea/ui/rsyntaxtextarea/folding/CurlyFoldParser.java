...
import jrcet.diycomponents.DiyJTextArea.ui.rsyntaxtextarea.TokenMaker;
 ...
 * If your {@link TokenMaker} uses a different token
					...
					if (getFoldableMultiLineComments() && t.isComment()) {
						...
						if (java) {
							...
							if (importStartLine>-1) {
								if (lastSeenImportLine>importStartLine) {
									...
									if (currentFold==null) {
										fold = new Fold(jrcet.diycomponents.DiyJTextArea.ui.rsyntaxtextarea.folding.FoldType.IMPORTS,
												textArea, importGroupStartOffs);
										folds.add(fold);
									}
									else {
										fold = currentFold.createChild(jrcet.diycomponents.DiyJTextArea.ui.rsyntaxtextarea.folding.FoldType.IMPORTS,
						...
						if (inMLC) {
							...
							if (t.endsWith(C_MLC_END)) {
								...
								if (currentFold==null) {
									currentFold = new Fold(jrcet.diycomponents.DiyJTextArea.ui.rsyntaxtextarea.folding.FoldType.COMMENT, textArea, mlcStart);
									currentFold.setEndOffset(mlcEnd);
									folds.add(currentFold);
									currentFold = null;
								}
								else {
									currentFold = currentFold.createChild(jrcet.diycomponents.DiyJTextArea.ui.rsyntaxtextarea.folding.FoldType.COMMENT, mlcStart);
									currentFold.setEndOffset(mlcEnd);
						...
						if (prevFold != null && line == lastRightCurlyLine) {
							...
							currentFold = new Fold(jrcet.diycomponents.DiyJTextArea.ui.rsyntaxtextarea.folding.FoldType.CODE, textArea, t.getOffset());
							folds.add(currentFold);
						}
						else { // A nested fold
