					...
					if (language>=0 && t.getType()==Token.SEPARATOR) {
						...
						if (t.startsWith(LANG_START[language])) {
							if (currentFold==null) {
								...
								currentFold = currentFold.createChild(jrcet.diycomponents.DiyJTextArea.ui.rsyntaxtextarea.folding.FoldType.CODE, t.getOffset());
							}
							inSublanguage = true;
						}
					...
					if (!inSublanguage) {
						...
						if (t.getType()==Token.COMMENT_MULTILINE) {
							...
							// Starting a MLC that ends on a later line...
								...
								if (currentFold==null) {
									currentFold = new Fold(jrcet.diycomponents.DiyJTextArea.ui.rsyntaxtextarea.folding.FoldType.COMMENT, textArea, t.getOffset());
									folds.add(currentFold);
								}
								else {
									currentFold = currentFold.createChild(jrcet.diycomponents.DiyJTextArea.ui.rsyntaxtextarea.folding.FoldType.COMMENT, t.getOffset());
								}
								inMLC = true;
							}
						...
						// If we're starting a new tag...
							...
							if (isFoldableTag(tagNameToken)) {
								...
								if (tagCloseToken.isSingleChar(Token.MARKUP_TAG_DELIMITER, '>')) {
									if (currentFold==null) {
										currentFold = new Fold(jrcet.diycomponents.DiyJTextArea.ui.rsyntaxtextarea.folding.FoldType.CODE, textArea,
