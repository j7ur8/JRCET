...
public class Intruder {
    ...
    private static JComponent tmpRootPanel;

    public static String invokeIntruder(byte[] currentPayload) {

        tmpPayload = new String(currentPayload, StandardCharsets.UTF_8);

        for(JComponent jComponent : IntruderModuleComponentList){
            tmpRootPanel = jComponent;
...
//            return tmpPayload;
    ...
    public static void rsaProcessor(){
        RSyntaxTextArea publicArea = (RSyntaxTextArea) Helper.getComponent(tmpRootPanel, "IntruderMainRsaPublicArea"); assert publicArea!=null;
        RSyntaxTextArea privateArea = (RSyntaxTextArea) Helper.getComponent(tmpRootPanel, "IntruderMainRsaPrivateArea"); assert privateArea!=null;
